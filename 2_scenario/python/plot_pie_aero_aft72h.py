...
DataPath = '/data/home/zzy/CHI_PartMC/baselineF2/out/'
FigurePath = DataPath
FigureName = 'aero_mass_pie_after72h'
...
#OM[71:]
OC_avg = sum(OC[71:])/len(OC[71:])*1e9
MSA_avg = sum(MSA[71:])/len(MSA[71:])*1e9
ARO1_avg = sum(ARO1[71:])/len(ARO1[71:])*1e9
ARO2_avg = sum(ARO2[71:])/len(ARO2[71:])*1e9
ALK1_avg = sum(ALK1[71:])/len(ALK1[71:])*1e9
OLE1_avg = sum(OLE1[71:])/len(OLE1[71:])*1e9
API1_avg = sum(API1[71:])/len(API1[71:])*1e9
API2_avg = sum(API2[71:])/len(API2[71:])*1e9
LIM1_avg = sum(LIM1[71:])/len(LIM1[71:])*1e9
LIM2_avg = sum(LIM2[71:])/len(LIM2[71:])*1e9
OIN_avg = sum(OIN[71:])/len(OIN[71:])*1e9
OM_avg = OC_avg+MSA_avg+ARO1_avg+ARO2_avg+ALK1_avg+OLE1_avg+API1_avg+API2_avg+LIM1_avg+LIM2_avg+OIN_avg

#BC SO4 NO3 NH3
BC_avg = sum(BC[71:])/len(BC[71:])*1e9
SO4_avg = sum(SO4[71:])/len(SO4[71:])*1e9
NO3_avg = sum(NO3[71:])/len(NO3[71:])*1e9
NH4_avg = sum(NH4[71:])/len(NH4[71:])*1e9

#aero_Sum
Aero_avg =sum(Aero[71:])/len(Aero[71:])*1e9
...
#name_list = ['OM-BC\u2099avg','BC_avg','NO3_avg','SO4_avg','NH4_avg']
name_list = ['OM avg','BC avg','NO$_3^-$ avg','SO$_4^{2-}$ avg','NH$_4^+$ avg']
#col_list = ['#A1A9D0','#9E9E9E','#F0988C','#F6CAE5','#96CCCB']
#col_list = ['#4eab90','#8eb69c','#edddc3','#eebf6d','#d94f33']
#col_list = ['#f4f1de','#df7a5e','#6b705c','#82b29a','#f2cc8e']
#col_list=['#0e606b','#45343d','#f66f69','#1597a5','#73bad6']
col_list=['#219ebc','#023047','#ffb703','#fb8402','#90c9e6']
#plt
plt.figure(figsize=(15,10),dpi=300)
patches, texts, autotexts=plt.pie(x=pie_data,labels=name_list,colors=col_list,center=(0,3),startangle=90,autopct='%.2f%%',labeldistance=None,pctdistance=1.15,textprops={'fontsize':15,'color' : 'black'})
plt.legend(patches,name_list,fontsize='15',loc='lower center', bbox_to_anchor=(0.5, -0.1),frameon=False,ncol=6)#,bbox_to_anchor=(0,0,0.5,1))
