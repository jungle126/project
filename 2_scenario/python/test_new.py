...
def nDp_to_filtered_nD(nDp,timeafter=96):
    ...
    BC_conc_SUM2_list.append(BC_conc_SUM2)
...
print(sum(norm_nDp40_60nm))
print(sum(norm_nDp60_80nm))
print(sum(norm_nDp80_100nm))
print(sum(norm_nDp100_120nm))
...
filtered_X_Dp40_60nm = [i for i, j in zip(x,n_DP_ln40_60nm) if j < 0 and i>=40] # do filter n>0 and Dp>=140nm
filtered_nDp40_60nm = [j for i, j in zip(x,n_DP_ln40_60nm) if j < 0 and i>=40]

filtered_X_Dp60_80nm = [i for i, j in zip(x,n_DP_ln60_80nm) if j < 0 and i>=60] # do filter n>0 and Dp>=140nm
filtered_nDp60_80nm = [j for i, j in zip(x,n_DP_ln60_80nm) if j < 0 and i>=60]
...
filtered_X_Dp = [i for i, j in zip(x,n_DP_ln) if j < 0 and i>=90 ] # do filter n>0 and Dp>=140nm
filtered_nDp = [j for i, j in zip(x,n_DP_ln) if j < 0 and i>=90]
...
fig, ax = plt.subplots(figsize=(3.5, 3.5),constrained_layout=False)
ax.scatter(filtered_X_Dp40_60nm, filtered_nDp40_60nm, s=5, color='#12cbc5', alpha=0.8 ,label = '$\mathrm{D_c}$ = 40-60 nm')
ax.scatter(filtered_X_Dp60_80nm, filtered_nDp60_80nm, s=5, color='#1593c3', alpha=0.8 ,label = '$\mathrm{D_c}$ = 60-80 nm')
ax.scatter(filtered_X_Dp80_100nm, filtered_nDp80_100nm, s=5, color='#0452d7', alpha=0.8 ,label = '$\mathrm{D_c}$ = 80-100 nm')
ax.scatter(filtered_X_Dp100_120nm, filtered_nDp100_120nm, s=5, color='#1c1464', alpha=0.8 ,label = '$\mathrm{D_c}$ = 100-120 nm')
