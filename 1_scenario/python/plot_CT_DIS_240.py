...
for n in range(len(n_CT_ln)):
    filtered_X_CT[n] = [i for i, j in zip(x,n_CT_ln[n]) if j < 0 and i>0]
    filtered_nCT[n] = [j for i, j in zip(x,n_CT_ln[n]) if j < 0 and i>0]
#coculate B and K
...
for i in range(len(X)):
    ...
    ax.scatter(plotx, ploty, s=5, color='red', alpha=0.8) #,label = 'shell')
#ax.plot(x1,y1,color = 'black',linestyle = '-.')
#ax.axvline(x=linefitmin, color='grey', linestyle='--', linewidth=1)
    ax.set_title(FigureName, fontsize = 12 ,loc = 'center')
