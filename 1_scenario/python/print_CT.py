...
y1 = x1*k+b
#k1 and b1
prefiltered_X_CT = [i for i, j in zip(x,ln_CONC_CT) if j < 0 and fitmin>=i>0 ]
prefiltered_nCT = [j for i, j in zip(x,ln_CONC_CT) if j < 0 and fitmin>=i>0 ]
k1,b1=optimize.curve_fit(f_1,prefiltered_X_CT,prefiltered_nCT)[0]
x2 = np.arange(0,fitmin,0.01)
y2 = x2*k1+b1
...
FigureName1 = 'Distribution of CT_all'
fig, ax = plt.subplots(figsize=(3.5, 3.5),constrained_layout=False)
ax.scatter(point_X,point_Y, s=5, color='red', alpha=1)
ax.plot(x1,y1,color = 'black',linestyle = '-.')
ax.plot(x2,y2,color = 'gray',linestyle = '-.')
ax.text(100,-8,'k1 = '+str(abs(round(k1,3))),fontsize=10,horizontalalignment='center', verticalalignment='center')
ax.text(520,-8,'k = '+str(abs(round(k,3))),fontsize=10,horizontalalignment='center', verticalalignment='center')
