...
for n in range(len(n_CT_ln)):
    filtered_X_CT[n] = [i for i, j in zip(x,n_CT_ln[n]) if j < 0]
    filtered_nCT[n] = [j for i, j in zip(x,n_CT_ln[n]) if j < 0]
filtered_X_CT_mv = [[] for i in range(len(n_CT_ln_mv))]
filtered_nCT_mv = [[] for i in range(len(n_CT_ln_mv))]
for n in range(len(n_CT_ln_mv)):
    filtered_X_CT_mv[n] = [i for i, j in zip(x,n_CT_ln_mv[n]) if j < 0]
    filtered_nCT_mv[n] = [j for i, j in zip(x,n_CT_ln_mv[n]) if j < 0]
#coculate B and K
k = [[] for i in range(len(norm_nCT))]
b = [[] for i in range(len(norm_nCT))]
for i in range(len(norm_nCT)):
    if filtered_X_CT[i] == []:k[i],b[i]=0,0
    else:
...
for i in range(len(norm_nCT_mv)):
    if filtered_X_CT_mv[i] == []:k_mv[i],b_mv[i]=0,0
    else:
...
ax.set_ylabel(r"Slope of CT distribution", fontsize=12)
